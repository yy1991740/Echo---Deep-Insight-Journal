...
3.  **配置环境变量 (至关重要!)**
    ...
    ```env
    # 你的火山引擎 API Key
    VITE_VOLC_API_KEY=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx

    # 高速模型接入点 ID (建议使用 Doubao-pro-4k 或 Doubao-lite)
    # 用于：即时反馈、每日一问
    VITE_FLASH_EP=ep-202xxxxxxxx-xxxxxx

    # 推理模型接入点 ID (建议使用 Doubao-pro-32k 或 Doubao-pro-128k)
    # 用于：深度心理分析、JSON 生成
    VITE_THINKING_EP=ep-202xxxxxxxx-xxxxxx
    ```
...
1.  将代码推送到 GitHub 仓库。
2.  在 Vercel 中导入该项目。
3.  **配置环境变量**：在 Vercel 项目设置 (Settings -> Environment Variables) 中，添加上述三个变量 (`VITE_VOLC_API_KEY`, `VITE_FLASH_EP`, `VITE_THINKING_EP`)。
4.  点击 Deploy。

*注：项目已包含 `vercel.json`，可自动处理单页应用 (SPA) 的路由重定向问题。*
