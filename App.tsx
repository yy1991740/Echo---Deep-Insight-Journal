...
import { getFastFeedback, getDeepInsight, getDailyQuestion, getStoredConfig } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const toggleRecording = useCallback(() => {
    const config = getStoredConfig();
    if (!config) {
        alert(language === 'zh' ? "未检测到 API 配置。请在 Vercel 环境变量中配置 VITE_VOLC_API_KEY。" : "Missing Vercel Environment Variables.");
        return;
    }
