...
export const getStoredConfig = (): ApiConfig | null => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (localApiKey && localFlashEp && localThinkingEp) {
      return {
        apiKey: localApiKey,
  ...
  // Strictly use Environment Variables (Vercel/Vite)
  // Note: import.meta.env type is augmented in types.ts
  const env = import.meta.env;
  
  // Use optional chaining carefully
  const envApiKey = env?.VITE_VOLC_API_KEY;
  const envFlashEp = env?.VITE_FLASH_EP;
  const envThinkingEp = env?.VITE_THINKING_EP;

  if (envApiKey && envFlashEp && envThinkingEp) {
    return {
      apiKey: envApiKey,
      flashEp: envFlashEp,
...
async function callDoubao(model: string, messages: any[], temperature: number = 0.7): Promise<string> {
  ...
  if (!config) {
    throw new Error("System Environment Variables missing. Please configure VITE_VOLC_API_KEY, VITE_FLASH_EP, and VITE_THINKING_EP in your Vercel project.");
  }
  ...
  try {
    const response = await fetch(BASE_URL, {
      ...
      body: JSON.stringify({
        model: model, // This MUST be the Endpoint ID (ep-xxxx), NOT the model name
        messages: messages,
    ...
    if (!response.ok) {
      ...
      // Handle 404 specifically (Wrong Endpoint ID)
      if (response.status === 404) {
          console.error("Volcengine 404 Error - Likely invalid Endpoint ID:", model);
          throw new Error(`Endpoint ID Not Found (404). Please check VITE_FLASH_EP / VITE_THINKING_EP in Vercel. Ensure it starts with 'ep-' and matches the ID in Volcengine Console.`);
      }
...
export const getFastFeedback = async (text: string, lang: Language): Promise<string> => {
  const config = getStoredConfig();
  if (!config) return lang === 'zh' ? "请检查 Vercel 环境变量配置。" : "Please check Vercel environment variables.";
